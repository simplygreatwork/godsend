	...
	Secure : Class.extend({
		...
		connect : function(callback) {										// authentication data will come from over the bus
			...
			this.bus.connect({
				...
				responded : function(properties) {							// next: now update broker and master user records (the patterns at least)
					this.connected = true;
		...
		exchange : function(request, response, connection) {
			...
			if (user) {
				if (user.isSendable(request.pattern)) {
					var receivables = 0;
									...
									if (array.length === receivables) {
										...
										array.forEach(function(each) {
											if (godsend.Utility.isHandled(JSON.parse(each.data))) {
												user = this.users[array[0].connection.credentials.username];
