...
godsend.Register = Class.extend({
	...
	validateReceiver : function(receiver) {
		
		receiver.id = this.ensure(receiver.id, uuid.v4());
		receiver.weight = this.ensure(receiver.weight, 0);
		...
		receiver.version.name = this.ensure(receiver.version.name, ' unversioned ');
	},
	...
	checkConflicts : function() {
		...
		return godsend.Utility.ensure(value, substitute);
	}
});
